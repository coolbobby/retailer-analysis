...
def build_workbook(pos_data, bcd_data, dbn_data, progress_callback=None):
    ...
    name_map = _build_per_franchise_sheets(
        wb, fields, active_rows, unique_vals, ws_grp, dbn_fields, dbn_by_username,
        pivot, day_values, header_font, header_fill, normal_font,
    )

    # --- Index (navigation for every sheet in the workbook) ---
    note("Building Index sheet...")
    _build_index_sheet(wb, unique_vals, name_map, active_attached_counts)
...
FIXED_SHEET_DESCRIPTIONS = [
    ("Username_Lookup", "Type a username to see its full profile, franchisee/division details, monthly Sims Sold, day-wise activations, locality sales, and ranking."),
    ("POS Details", "Every POS record from the source file, ACTIVE and DEACTIVE alike."),
    ("Active_POS", "Only POS records where state = ACTIVE."),
    ("Franchisee_CSC_Wise", "Count of ACTIVE POS per Franchisee_CSC code, with links to each Franchisee_CSC sheet."),
    ("Monthly_Activations", "Raw import of the BCD (SIM activation) file for the month."),
    ("Daywise_Pivot", "Per-username, per-day activation counts, pre-aggregated from Monthly_Activations."),
    ("Retailer_DBN_Division_Wise", "Raw import of the Retailer DBN Division Wise file."),
    ("Locality_Wise_Sales", "Total Sims Sold this month per DBN Locality."),
    ("POS_Sims_Ranking", "Every ACTIVE POS username ranked by Sims Sold this month, with POS/franchisee details."),
]


def _build_index_sheet(wb, unique_vals, name_map, active_attached_counts):
    ws_idx = wb.create_sheet("Index")
    ws_idx.merge_cells("A1:C1")
    idx_title = ws_idx["A1"]
    idx_title.value = "WORKBOOK INDEX"
    idx_title.font = Font(name="Arial", bold=True, color="FFFFFF", size=14)
    idx_title.fill = PatternFill(start_color="1F3864", end_color="1F3864", fill_type="solid")
    idx_title.alignment = Alignment(horizontal="center", vertical="center")
    ws_idx.row_dimensions[1].height = 26

    ws_idx.append([])
    ws_idx.append(["#", "Sheet", "Description"])
    for cell in ws_idx[3]:
        cell.font = Font(name="Arial", bold=True, color="FFFFFF")
        cell.fill = PatternFill(start_color="305496", end_color="305496", fill_type="solid")
        cell.alignment = Alignment(horizontal="center")

    idx_light_fill = PatternFill(start_color="F2F2F2", end_color="F2F2F2", fill_type="solid")
    row = 4
    n = 1
    for sheet_name, desc in FIXED_SHEET_DESCRIPTIONS:
        ws_idx.cell(row=row, column=1, value=n).font = Font(name="Arial")
        link_cell = ws_idx.cell(row=row, column=2, value=sheet_name)
        link_cell.hyperlink = f"#'{sheet_name}'!A1"
        link_cell.font = Font(name="Arial", color="0563C1", underline="single", bold=True)
        ws_idx.cell(row=row, column=3, value=desc).font = Font(name="Arial")
        if (n % 2) == 0:
            for c in range(1, 4):
                ws_idx.cell(row=row, column=c).fill = idx_light_fill
        row += 1
        n += 1
    for val in unique_vals:
        safe_name = name_map[val]
        pos_count = active_attached_counts.get(val, 0)
        ws_idx.cell(row=row, column=1, value=n).font = Font(name="Arial")
        link_cell = ws_idx.cell(row=row, column=2, value=safe_name)
        link_cell.hyperlink = f"#'{safe_name}'!A1"
        link_cell.font = Font(name="Arial", color="0563C1", underline="single", bold=True)
        ws_idx.cell(row=row, column=3,
                     value=f'Franchisee_CSC "{val}" — {pos_count} ACTIVE POS record(s).').font = Font(name="Arial")
        if (n % 2) == 0:
            for c in range(1, 4):
                ws_idx.cell(row=row, column=c).fill = idx_light_fill
        row += 1
